...
export function CaseAddModal({ groupId, existingCategories, onSave, onClose }: CaseAddModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/60 w-full max-w-md max-h-full flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] shrink-0">
          <h2 className="text-white font-semibold text-base">{t("case.new")}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          <div>
            <p className="text-xs font-semibold text-gray-400 mb-1.5">{t("caseAdd.caseName")}</p>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t("caseAdd.caseNamePlaceholder")} className={inputClass} autoFocus />
          </div>
          <div>
            ...
            <input
              ...
              placeholder={t("caseAdd.categoryPlaceholder")}
          ...
          <div className="border-t border-white/[0.06] pt-3">
            <p className="text-xs font-semibold text-gray-400 mb-1.5">{t("caseAdd.firstVariant")}</p>
            <div className="space-y-2">
              <input type="text" value={algName} onChange={(e) => setAlgName(e.target.value)} placeholder={t("variant.namePlaceholder")} className={inputClass} />
              <input
                type="text"
                value={alg}
              ...
              />
              <input type="url" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} placeholder={t("caseEdit.youtubePlaceholder")} className={inputClass} />
            </div>
          </div>
        </div>
